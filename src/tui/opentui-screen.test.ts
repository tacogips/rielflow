  ...
  test("ignores removed legacy workflowCalls in the concise workflow summary", () => {
    ...
    const loaded: LoadedWorkflow = {
      ...
      bundle: {
        ...
        } as unknown as LegacyWorkflowCallWorkflow,
      },
    };
    ...
    expect(content).toContain("Cross-workflow dispatches: 0");
...
describe("workflow preview text helpers", () => {
  ...
  test("buildWorkflowHistoryHeader omits the description line when it is absent", () => {
    ...
    expect(text).toContain("nodes=3  crossWorkflowDispatches=0");
  ...
  test("buildWorkflowHistoryHeader counts step-derived workflow calls for step-addressed bundles", () => {
    ...
    expect(text).toContain("nodes=2  crossWorkflowDispatches=1");
    expect(text).not.toContain("subWorkflows=");
  });
});
