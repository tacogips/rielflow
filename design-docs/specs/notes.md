...
- Manager auth/idempotency contract: GraphQL manager mutations use a runtime-issued bearer token scoped to one manager session and enforce persisted idempotency by `(mutationName, managerSessionId, idempotencyKey)`. The idempotency row must be atomically claimed before manager-control side effects run; same-key/same-hash concurrent callers wait for or read the completed response or stored failure, while same-key/different-hash callers fail before side effects.
...
- Node add-on contract: `workflow.json.nodes[]` may reference built-in or installed worker add-ons with `addon` instead of `nodeFile`; the loader resolves them into effective node payloads while save/edit surfaces preserve the authored reference. Built-in add-ons currently include `rielflow/chat-reply-worker`, `rielflow/codex-worker`, `rielflow/claude-code-worker`, `rielflow/x-gateway-read`, `rielflow/x-gateway`, `rielflow/mail-gateway-read`, and `rielflow/mail-gateway`. Installed `kind: "node-addon"` packages may provide declarative or executable add-ons; executable add-ons require declared `execution` metadata, verified `contentDigest` and package `integrity`, explicit capabilities, and preserved dependency locks. `addon.inputs` supplies invocation-specific node variables, and only descriptors that consume explicit environment bindings may accept `addon.env`; required add-on env sources are runtime readiness prerequisites.
