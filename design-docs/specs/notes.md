...
- Conversation handoff: `divedra` routes by explicit `OutputRef` (`workflowExecutionId`, `outputNodeId`, `nodeExecId`) instead of implicit latest-output inference.
...
- Legacy compatibility review note (2026-04-25, updated 2026-04-26): the repository does not yet fully match the no-backcompat target. The public `call-node` surface is removed; CLI `workflow inspect`, GraphQL workflow views, and `buildInspectionSummary` are step-first (`entryStepId`, `managerStepId`, `stepIds`, node registry ids) without node-addressed entry/manager fields on those summaries. Manager-control no longer includes structural `start-sub-workflow` / `deliver-to-child-input` actions; child input and root sub-workflow starts are engine-planned only. Manager-control accepts only `retry-step` / `execute-optional-step` / `skip-optional-step` with `stepId` (node-id action aliases are removed). `call-step` direct execution now also uses a step-addressed internal executor input (`stepId`) instead of a node-shaped wrapper boundary. Step-addressed validation rejects top-level authored `workflow.workflowCalls` (cross-workflow calls use step transitions); legacy node-graph bundles may still carry `workflowCalls` until the compatibility validator path is removed. Runtime/session/output-ref metadata projection is now centralized around the shared step-identity and output-ref helpers (`StepIdentityFields`, `toStepIdentityFields(...)`, `buildOutputRefForExecution(...)`) instead of re-stating the same `stepId` / `nodeRegistryId` / mailbox payload shape in each helper. Remaining debt is mostly in normalized `WorkflowJson` synthesis, engine `workflowCallsForExecutionMatch` over step-derived calls, structural `subWorkflows` in normalized execution, and TUI lines for **non-step-addressed** loads. Auto-improve supervision remediation is step-only (`rerunFromStepId` on the engine); `toStepAddressedWorkflowForSupervision` projects legacy node-graph targets for planning. Iterations should delete compatibility paths per `impl-plans/workflow-legacy-compatibility-removal.md` rather than extend them.
...
- Auto-improve supervision direction: `divedra` supports an `auto improve mode` with two shipped paths: the default engine-owned supervision loop and an opt-in nested superviser workflow path enabled with `--nested-superviser`, both using the same persisted audit model.
...
- Node add-on contract: `workflow.json.nodes[]` may reference built-in worker add-ons with `addon` instead of `nodeFile`; the loader resolves them into effective node payloads while save/edit surfaces preserve the authored reference. Built-in add-ons currently include `divedra/chat-reply-worker`, `divedra/codex-worker`, `divedra/claude-code-worker`, `divedra/x-gateway-read`, `divedra/x-gateway`, `divedra/mail-gateway-read`, and `divedra/mail-gateway`. `addon.inputs` supplies invocation-specific node variables, and only descriptors that consume explicit environment bindings may accept `addon.env`; required add-on env sources are runtime readiness prerequisites.
