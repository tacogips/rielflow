...
- GraphQL transport contract: `/graphql` accepts standard JSON request envelopes with `query` and optional `variables`, and `rielflow graphql` executes locally in-process when no endpoint is provided. `--endpoint` or `DIVEDRA_GRAPHQL_ENDPOINT` selects remote HTTP transport.
...
- Node add-on contract: `workflow.json.nodes[]` may reference built-in worker add-ons with `addon` instead of `nodeFile`; the loader resolves them into effective node payloads while save/edit surfaces preserve the authored reference. Built-in add-ons currently include `rielflow/chat-reply-worker`, `rielflow/codex-worker`, `rielflow/claude-code-worker`, `rielflow/x-gateway-read`, `rielflow/x-gateway`, `rielflow/mail-gateway-read`, and `rielflow/mail-gateway`. `addon.inputs` supplies invocation-specific node variables, and only descriptors that consume explicit environment bindings may accept `addon.env`; required add-on env sources are runtime readiness prerequisites.
