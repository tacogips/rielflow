...
- `cli workflow run <name>`
  ...
  - Registry-backed runs accept `--registry <registry-url-or-id>` and `--branch <branch>` with the same meaning as package checkout. The temporary checkout must validate package metadata, checksum/integrity, and the workflow bundle before execution.
  ...
  - The temporary checkout must not write normal project/user checkout provenance or install package skills. Execution metadata should record enough source provenance for audit: package id, workflow name, registry URL/id, branch/ref, source directory, checksum, checksum algorithm, and temporary workflow directory.
...
| `--from-registry`                                 | boolean       | `false`                                             | For `workflow run`: interpret the positional workflow name as a registry package id, perform a temporary non-persistent checkout, run from that checkout, then clean it up                                                 |
| `--registry`                                      | string        | default registry                                    | For workflow package checkout/search and `workflow run --from-registry`: restrict package resolution to a registry id or canonical GitHub registry URL                                                                     |
| `--branch`                                        | string        | registry default branch                             | For workflow package checkout/search and `workflow run --from-registry`: select the registry branch/ref used for package metadata and content                                                                              |
