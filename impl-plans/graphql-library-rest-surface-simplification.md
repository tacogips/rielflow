...
**Blockers**: None
**Notes**: Added the standard `graphql` dependency and replaced the handwritten `/graphql` parser/executor with a library-backed schema execution path while keeping the existing domain resolvers. Removed legacy workflow/session REST routes from `serve`, updated the browser session query for spec-compliant GraphQL field selection, removed the file-harness REST compatibility paths, and verified with `bun run test`, `bun run typecheck`, `bun run build`, and a live `agent-browser open/snapshot/screenshot/close` pass against `divedra serve`.
