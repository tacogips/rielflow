...
function resolveNearestWorkflowProjectRoot(cwd: string): string {
  ...
  while (true) {
    ...
    try {
      ...
      // Keep walking upward when `.rielflow` is absent or unreadable.
    }
 ...
 * `<user-root>/artifacts`, where user root defaults to `~/.rielflow`.
 */
export function computeDefaultRootDataDir(userRoot?: string): string {
  const resolvedUserRoot = expandLeadingHome(userRoot ?? "~/.rielflow");
...
export function computeProjectScopedRootDataDir(
  ...
  const resolvedUserRoot = resolveRootPath(
    expandLeadingHome(input.userRoot ?? "~/.rielflow"),
    cwd,
  );
  return path.join(
    resolvedUserRoot,
...
export function resolveProjectRootForScopeRoot(scopeRoot: string): string {
  ...
  return path.basename(resolvedScopeRoot) === ".rielflow"
