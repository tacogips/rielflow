...
export interface NodeExecutionMailboxMeta {
  ...
  readonly managerControl?: {
    readonly preferredTransport: "rielflow gql";
...
function buildManagerControlMetadata(input: {
  ...
  return {
    preferredTransport: "rielflow gql",
...
function renderManagerControlSection(
  ...
  return [
    ...
    "When `rielflow gql` is available in the execution environment, prefer typed GraphQL manager actions over freeform control prose.",
    "Use payload `managerControl` when typed GraphQL manager actions are unavailable for that execution backend (e.g. no `rielflow gql` in the environment).",
