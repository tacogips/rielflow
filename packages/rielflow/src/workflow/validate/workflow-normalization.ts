...
export function normalizeStepAddressedWorkflow(
  ...
  if (promptsRaw !== undefined) {
    if (!isRecord(promptsRaw)) {
      ...
      const rielflowPromptTemplateRaw = promptsRaw["rielflowPromptTemplate"];
      const workerSystemPromptTemplateRaw =
        promptsRaw["workerSystemPromptTemplate"];

      if (
        rielflowPromptTemplateRaw !== undefined &&
        typeof rielflowPromptTemplateRaw !== "string"
      ) {
        issues.push(
          makeIssue(
            "error",
            "workflow.prompts.rielflowPromptTemplate",
      ...
      prompts = {
        ...(typeof rielflowPromptTemplateRaw === "string"
          ? { rielflowPromptTemplate: rielflowPromptTemplateRaw }
          : {}),
        ...(typeof workerSystemPromptTemplateRaw === "string"
