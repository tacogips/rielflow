...
async function makeTempDir(): Promise<string> {
  const directory = await mkdtemp(
    path.join(os.tmpdir(), "rielflow-paths-test-"),
  );
...
describe("runtime storage paths", () => {
  test("defaults non-scoped runtime data to the user artifacts root", async () => {
    ...
    const rootDataDir = path.join(os.homedir(), ".rielflow", "artifacts");
  ...
  test("builds project-scoped runtime data below the user root namespace", async () => {
    ...
    const userRoot = path.join(cwd, "user-home", ".rielflow");
  ...
  test("resolves project identity from both .rielflow and direct scope roots", () => {
    const projectRoot = path.join(os.tmpdir(), "workspace", "repo");

    expect(
      resolveProjectRootForScopeRoot(path.join(projectRoot, ".rielflow")),
    ).toBe(projectRoot);
  ...
  test("builds project-scoped runtime data from project scope roots", async () => {
    const cwd = await makeTempDir();
    const projectRoot = path.join(cwd, "workspace", "repo");
    const scopeRoot = path.join(projectRoot, ".rielflow");
