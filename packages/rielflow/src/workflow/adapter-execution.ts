...
function isAdapterFailureCode(value: unknown): value is AdapterFailureCode {
  return (
...
function isAdapterProcessLogArray(
  value: unknown,
): value is readonly AdapterProcessLog[] {
  return (
    Array.isArray(value) &&
    value.every(
      (entry) =>
...
function toAdapterExecutionFailureLike(
  error: unknown,
): AdapterExecutionFailure | undefined {
  if (typeof error !== "object" || error === null) {
    return undefined;
  }
  if (!("code" in error) || !isAdapterFailureCode(error.code)) {
    return undefined;
  }
  ...
  return {
    code: error.code,
    message: error.message,
    ...(!("processLogs" in error) ||
    error.processLogs === undefined ||
    !isAdapterProcessLogArray(error.processLogs)
      ? {}
      : { processLogs: error.processLogs }),
  };
}
...
function toExecutionFailure(
  ...
  const adapterErrorLike = toAdapterExecutionFailureLike(error);
  if (adapterErrorLike !== undefined) {
    return adapterErrorLike;
  }
...
const TIMED_OUT_PACKAGE_LOG_GRACE_MS = 2_000;
...
async function waitForTimedOutPackageExecutionLogs(input: {
  readonly execution: Promise<AdapterExecutionOutput>;
  readonly timeoutMessage: string;
}): Promise<AdapterExecutionFailure | undefined> {
  const timeoutLogResult = await Promise.race([
    input.execution.then(
      (output): AdapterExecutionFailure | undefined =>
        output.processLogs === undefined
          ? undefined
          : {
              code: "timeout",
              message: input.timeoutMessage,
              processLogs: output.processLogs,
            },
      (error: unknown): AdapterExecutionFailure | undefined => {
        if (error instanceof AdapterExecutionError) {
          return toAdapterExecutionFailure(error);
        }
        return toAdapterExecutionFailureLike(error);
      },
    ),
    new Promise<undefined>((resolve) => {
      setTimeout(resolve, TIMED_OUT_PACKAGE_LOG_GRACE_MS);
    }),
  ]);
...
export async function executePackageNodeWithTimeout(
  ...
  const stall = supervisionStall
    ? attachSupervisionStallToAbort(timeout.controller, supervisionStall)
    : undefined;
  let packageExecution: Promise<AdapterExecutionOutput> | undefined;

  try {
    const loadExecutor = dependencies.loadExecutor ?? loadPackageNodeExecutor;
    packageExecution = (async (): Promise<AdapterExecutionOutput> => {
    ...
    const timedOut =
      timeout.expired() ||
      (error instanceof AdapterExecutionError && error.code === "timeout");
    if (timedOut && packageExecution !== undefined) {
      const timeoutFailureWithLogs = await waitForTimedOutPackageExecutionLogs({
        execution: packageExecution,
        timeoutMessage,
      });
      if (timeoutFailureWithLogs !== undefined) {
        return err(timeoutFailureWithLogs);
      }
      void packageExecution.catch(() => undefined);
    }
    return err(
      toExecutionFailure(error, {
        timeoutMessage,
