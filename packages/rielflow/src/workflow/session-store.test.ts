...
async function makeTempDir(): Promise<string> {
  const directory = await mkdtemp(
    path.join(os.tmpdir(), "rielflow-session-store-test-"),
  );
...
describe("session-store", () => {
  test("uses ~/.rielflow/artifacts/sessions when unset", async () => {
    ...
    expect(resolved).toBe(
      path.join(os.homedir(), ".rielflow", "artifacts", "sessions"),
    );
  });
  ...
  test("uses user-root artifacts for default session-store scoping", async () => {
    ...
    await mkdir(path.join(root, ".rielflow"), { recursive: true });
  ...
  test("save/load roundtrip preserves supervision state", async () => {
    ...
    const session: ReturnType<typeof createSessionState> = {
      ...
      supervision: {
        ...
        targetWorkflowId: "wf",
        superviserWorkflowId: "rielflow-superviser",
        status: "running",
        attemptCount: 2,
  ...
  test("save/load roundtrip preserves fanout workspace lineage", async () => {
          ...
          branches: [
            {
              ...
              workspaceRoot: "/tmp/rielflow-fanout-workspaces/old-branch",
            },
            {
              branchIndex: 1,
              item: { id: "feature-b" },
              ...
              workspaceRoot: "/tmp/rielflow-fanout-workspaces/old-branch-b",
            },
          ],
        },
        {
          ...
          branches: [
            {
              ...
              workspaceRoot: "/tmp/rielflow-fanout-workspaces/new-branch",
              supersededWorkspaceRoot:
                "/tmp/rielflow-fanout-workspaces/old-branch",
            },
          ],
        },
      ],
    ...
    expect(retryBranch?.workspaceRoot).toBe(
      "/tmp/rielflow-fanout-workspaces/new-branch",
    );
