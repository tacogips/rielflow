...
async function writeMinimalWebhookFixture(input: {
  ...
  const workflowRoot = path.join(input.root, ".rielflow");
  const eventRoot = path.join(input.root, ".rielflow-events");
  ...
  test("verifies signed webhook bodies before parsing JSON", async () => {
    ...
    const response = await capturedFetch(
      new Request("http://127.0.0.1/events/chat", {
        method: "POST",
        headers: {
          "x-rielflow-signature": createHmac("sha256", "wrong-secret")
            .update("{")
            .digest("hex"),
        },
        body: "{",
