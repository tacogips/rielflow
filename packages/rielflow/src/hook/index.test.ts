...
function createGeminiHookPayload(
  ...
  return createHookPayload({
    ...
    transcript_path: "/tmp/rielflow/gemini-transcript.json",
...
async function createRootDataDir(): Promise<string> {
  return mkdtemp(path.join(os.tmpdir(), "rielflow-hook-events-"));
}

function createRielflowHookEnv(
...
describe("runHookCommand", () => {
  ...
  test("required hook recording fails when rielflow context is missing", async () => {
    const capture = createIoCapture();
    const exitCode = await runHookCommand({
      deps: {
    ...
    expect(exitCode).toBe(1);
    expect(capture.stderr[0]).toContain("missing rielflow hook context");
  });
});
