...
async function makeTempDir(): Promise<string> {
  const directory = await mkdtemp(
    path.join(os.tmpdir(), "rielflow-graphql-schema-test-"),
  );
...
async function createWorkflowCallWorkflowFixture(root: string) {
  ...
  await mkdir(path.join(workflowDir, "scripts"), { recursive: true });
  await writeFile(
  ...
  await mkdir(path.join(reviewDir, "scripts"), { recursive: true });
  await writeFile(
  ...
  for (const scriptPath of [
    path.join(workflowDir, "scripts", "main-worker.sh"),
    path.join(reviewDir, "scripts", "reviewer.sh"),
  ]) {
...
describe("createGraphqlSchema", () => {
  ...
  test("fixed resolved workflow source pins scoped duplicate overview rows", async () => {
    ...
    const projectScopeRoot = path.join(workspace, ".rielflow");
    const projectWorkflowRoot = path.join(projectScopeRoot, "workflows");
    const userScopeRoot = path.join(base, "userhome", ".rielflow");
            ...
            branches: [
              {
                ...
                workspaceRoot: "/tmp/rielflow/fanout/new",
                supersededWorkspaceRoot: "/tmp/rielflow/fanout/old",
              },
              {
                branchIndex: 1,
                item: { id: "feature-b" },
  ...
  test("reports scaffolded managed starters as not runtime-ready until code-manager runtime lands", async () => {
    ...
    expect(workflow?.managerStepId).toBe("rielflow-manager");
    expect(workflow?.entryStepId).toBe("rielflow-manager");
    expect(workflow?.stepIds).toEqual(["rielflow-manager", "main-worker"]);
    ...
    expect(workflow?.runtime.requirements).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          sourceStepIds: ["rielflow-manager"],
        }),
      ]),
    );
