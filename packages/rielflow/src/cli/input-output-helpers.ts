...
export function printHelp(io: CliIo): void {
  ...
  io.stdout(
    "  rielflow session health <workflow-execution-id> [--live] [--stall-timeout-ms <n>] [--log-limit <n>] [--include-llm-messages] [--llm-limit <n>] [options]",
  );
  io.stdout(
    "  rielflow session rerun <workflow-execution-id> <step-id> [options]",
  );
  io.stdout(
    "  rielflow session continue <workflow-execution-id> --start-step <step-id> --after-step-run <step-run-id> [options]",
  );
  io.stdout(
    "  rielflow session export <workflow-execution-id> [--file <path>] [options]",
  );
  io.stdout(
    "  rielflow session logs <workflow-execution-id> [--format <text|json|jsonl>] [options]",
  );
  io.stdout(
    "  rielflow session step-runs <workflow-execution-id> [--step <step-id>] [--status <status>] [options]",
  );
  io.stdout(
    "  rielflow serve [workflow-name] [--host <host>] [--port <port>] [--read-only] [--no-exec]",
  );
  io.stdout(
    "  rielflow graphql <graphql-document> [--variables <json|@file>] [--endpoint <url>] [--auth-token <token>]",
  );
  io.stdout(
    "  rielflow events <validate|serve|emit|list|replay|replies|schedules> [source-id|receipt-id|workflow-execution-id] [--event-root <path>] [--event-file <path>]",
  );
  io.stdout(
    "  rielflow events schedules <list|inspect|cancel> [schedule-id] [--source <id>] [--status <status>] [--limit <n>]",
  );
  io.stdout(
    "  rielflow call-step <workflow-id> <workflow-run-id> <step-id> [--message-json <json> | --message-file <path>] [--prompt-variant <name>] [--continue-session] [--timeout-ms <ms>] [--resume-step-exec <id>] [options]",
  );
  io.stdout(`  rielflow hook [--vendor ${HOOK_VENDOR_USAGE}]`);
  io.stdout(`  rielflow hook snippet --vendor ${HOOK_VENDOR_USAGE}`);
  io.stdout("");
  ...
  io.stdout(
    "  --supervisor-workflow <id> Superviser bundle id (alias: --superviser-workflow; persisted; rielflow/* control + optional nested driver)",
  );
  io.stdout(
