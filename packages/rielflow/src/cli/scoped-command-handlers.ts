...
import type { WorkflowScheduleStatus } from "rielflow-events";
import {
...
export async function runCliGraphqlScope(
  ...
  if (document.length === 0) {
    ...
    io.stderr("usage: rielflow graphql <graphql-document> [options]");
    return 2;
  }
...
export async function runCliHookScope(
  ...
  if (command !== undefined) {
    if (command !== "snippet") {
      ...
      io.stderr(`usage: rielflow hook snippet --vendor ${HOOK_VENDOR_USAGE}`);
      return 2;
    }
    if (target !== undefined) {
  ...
  if (positionals.length > 1) {
    ...
    io.stderr(`usage: rielflow hook [--vendor ${HOOK_VENDOR_USAGE}]`);
    return 2;
  }
...
export async function runCliEventsScope(
  ...
  if (command === "emit") {
    ...
    if (sourceId === undefined || eventFile === undefined) {
      ...
      io.stderr(
        "usage: rielflow events emit <source-id> --event-file <path> [options]",
      );
      return 2;
    }
    try {
  ...
  if (command === "schedules") {
    ...
    if (schedulesCommand === "inspect") {
      if (scheduleId === undefined) {
        ...
        io.stderr("usage: rielflow events schedules inspect <schedule-id>");
        return 2;
      }
      try {
    ...
    if (schedulesCommand === "cancel") {
      if (scheduleId === undefined) {
        io.stderr("schedule id is required");
        io.stderr(
          "usage: rielflow events schedules cancel <schedule-id> [--reason <text>]",
        );
        return 2;
      }
      try {
    ...
    io.stderr(
      "usage: rielflow events schedules <list|inspect|cancel> [schedule-id]",
    );
    return 2;
  }
  ...
  if (command === "replay") {
    ...
    if (receiptId === undefined) {
      ...
      io.stderr(
        "usage: rielflow events replay <receipt-id> [--reason <text>] [--dry-run] [options]",
      );
      return 2;
    }
    try {
...
export async function runCliCallStepScope(
  ...
  if (
    ...
    io.stderr(
      "usage: rielflow call-step <workflow-id> <workflow-run-id> <step-id> [--message-json <json> | --message-file <path>] [--prompt-variant <name>] [--continue-session] [--timeout-ms <ms>] [--resume-step-exec <id>] [options]",
    );
    return 2;
  }
