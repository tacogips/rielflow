...
async function createWorkflowCheckoutFetch(input: {
  ...
  const repoDirectory = `.rielflow/workflows/${input.workflowName}`;
  const files = [
    "workflow.json",
...
async function createWorkflowCallInspectFixture(
  ...
  await writeJson(
    path.join(workflowDirectory, "nodes", "node-rielflow-manager.json"),
    {
      id: "rielflow-manager",
      executionBackend: "official/openai-sdk",
      model: "gpt-5",
  ...
  await writeJson(
    path.join(workflowDirectory, "nodes", "node-main-worker.json"),
    {
      id: "main-worker",
      nodeType: "command",
      command: { scriptPath: "scripts/main-worker.sh" },
      variables: {},
    },
  );
  await writeJson(
    path.join(workflowDirectory, "nodes", "node-post-review.json"),
    {
      id: "post-review",
      nodeType: "command",
      command: { scriptPath: "scripts/post-review.sh" },
      variables: {},
    },
  );
  ...
  await mkdir(path.join(reviewDirectory, "scripts"), { recursive: true });
  ...
  await writeJson(path.join(reviewDirectory, "nodes", "node-reviewer.json"), {
    id: "reviewer",
    nodeType: "command",
    command: { scriptPath: "scripts/reviewer.sh" },
    variables: {},
  });
  for (const scriptPath of [
    path.join(workflowDirectory, "scripts", "main-worker.sh"),
    path.join(workflowDirectory, "scripts", "post-review.sh"),
    path.join(reviewDirectory, "scripts", "reviewer.sh"),
  ]) {
...
describe("runCli", () => {
  ...
  test("returns help for unknown scope", async () => {
    ...
    expect(help).toContain("rielflow graphql <graphql-document>");
    expect(help).not.toContain("rielflow gql");
  ...
  test("create -> validate -> inspect roundtrip", async () => {
    ...
    expect(parsed.entryStepId).toBe("rielflow-manager");
    expect(parsed.managerStepId).toBe("rielflow-manager");
    expect(parsed.stepIds).toEqual(["rielflow-manager", "main-worker"]);
    expect(parsed.nodeRegistryIds).toEqual(["rielflow-manager", "main-worker"]);
    ...
    expect(inspectTextCapture.stdout.join("\n")).toContain(
      "stepIds: rielflow-manager, main-worker",
    );
    expect(inspectTextCapture.stdout.join("\n")).toContain(
      "nodeRegistryIds: rielflow-manager, main-worker",
    );
    expect(inspectTextCapture.stdout.join("\n")).toMatch(
  ...
  test("session resume restores and cleans paused registry temporary checkout", async () => {
    const root = await makeTempDir();
    const registryRoot = path.join(root, "registry");
    const userRoot = path.join(root, "user", ".rielflow");
    const packageRoot = path.join(registryRoot, "packages", "temp-resume-flow");
    await mkdir(packageRoot, { recursive: true });
    const created = await createWorkflowTemplate("temp-resume-flow", {
      workflowRoot: packageRoot,
    });
    expect(created.ok).toBe(true);
    if (!created.ok) {
    ...
    await writeFile(
      ...
      `${JSON.stringify(
        {
          name: "temp-resume-flow",
          version: "1.0.0",
          description: "Temporary resumable CLI run package",
          tags: ["cli"],
          workflow: {
            description: "Temporary resumable CLI run package",
            tags: ["cli"],
          ...
          checksum: "pending",
          checksumAlgorithm: "md5",
          workflowDirectory: "temp-resume-flow",
          backends: ["codex-agent"],
        },
        null,
        2,
    ...
    const temporaryWorkflowDirectory =
      runPayload.registrySource.package.temporaryWorkflowDirectory;
    const temporaryRoot = path.dirname(
      path.dirname(temporaryWorkflowDirectory),
    );
    await expect(realpath(temporaryWorkflowDirectory)).resolves.toEqual(
      expect.any(String),
    );

    const resumeCapture = createIoCapture();
    const resumeCode = await withLegacyWorkflowAuthorshipForCli(() =>
      runCli(
        [
          "session",
          "resume",
    ...
    expect(
      resumeCode,
      `stdout:\n${resumeCapture.stdout.join("\n")}\nstderr:\n${resumeCapture.stderr.join("\n")}`,
    ).toBe(0);
    const resumePayload = JSON.parse(resumeCapture.stdout.join("\n")) as {
      status: string;
      registrySource: {
    ...
    expect(resumePayload.registrySource.cleanup).toEqual({
      ok: true,
      remainingPaths: [],
    });
    await expect(
  ...
  test("workflow status and session commands share project-scoped storage inferred from workflow-definition-dir", async () => {
    ...
    const projectScopeRoot = path.join(workspaceRoot, ".rielflow");
  ...
  test("workflow status and session commands share direct workflow-definition storage outside project scopes", async () => {
    const root = await makeTempDir();
    const workflowRoot = path.join(root, "rielflow-workflows");
    const userRoot = path.join(root, "home", ".rielflow");
  ...
  test("workflow list labels scoped rows and warns on project user duplicates", async () => {
    const root = await makeTempDir();
    const workspaceRoot = path.join(root, "workspace");
    const projectRoot = path.join(workspaceRoot, ".rielflow");
    const userRoot = path.join(root, "home", ".rielflow");
                ...
                workflows: [
                  {
                    workflowName: "dup",
                    sourceScope: "project",
                    workflowDirectory: "/project/.rielflow/workflows/dup",
  ...
  test("workflow commands resolve explicit project scope root", async () => {
    const root = await makeTempDir();
    const projectRoot = path.join(root, "workspace", ".rielflow");
    const externalRoot = await makeTempDir();
    const previousCwd = process.cwd();
    process.chdir(externalRoot);
    try {
  ...
  test("inspect reports worker-only workflows without an authored manager node", async () => {
    await withLegacyWorkflowAuthorshipForCli(async () => {
      ...
      expect(textOut).toContain(
        `inline-json: rielflow workflow run worker-only --variables '{"request":""}'`,
      );
      expect(textOut).toContain(
        "explicit-file: rielflow workflow run worker-only --variables @./variables.json",
      );
      expect(textOut).toContain(
        "file-path: rielflow workflow run worker-only --variables ./variables.json",
      );
    });
  });
  ...
  test("project workflow run stores runtime data in user-root project namespace", async () => {
    ...
    const projectScopeRoot = path.join(projectRoot, ".rielflow");
    const workflowRoot = path.join(projectScopeRoot, "workflows");
    const userRoot = path.join(projectRoot, "user-home", ".rielflow");
  ...
  test("session logs prints runtime node and communication logs as jsonl", async () => {
    ...
    expect(communicationLog?.message).toContain(
      "transition rielflow-manager -> main-worker when always",
    );
  ...
  test("workflow run keeps the runtime db aligned with explicit storage roots", async () => {
    ...
    expect(runCode).toBe(4);
    expect(await Bun.file(path.join(root, "rielflow.db")).exists()).toBe(true);
    expect(await Bun.file(path.join(ambientRoot, "rielflow.db")).exists()).toBe(
      false,
    );
  });
      ...
      capture.io,
      {
        ...
        fetchImpl: async (input, init) => {
          ...
          requestedManagerSessionId =
            new Headers(init?.headers).get("x-rielflow-manager-session-id") ??
            "";
  ...
  test("local session continue uses retained registry checkout and cleans terminal result", async () => {
    ...
    const sourceSessionId = "sess-registry-continue-src";
    const session = createSessionState({
    ...
    const code = await runCli(["hook", "--vendor", "codex"], capture.io, {
      ...
      readStdin: async () =>
        JSON.stringify({
          ...
          transcript_path: "/tmp/rielflow/transcript.jsonl",
          cwd: "/tmp/rielflow",
  ...
  test("hook command rejects positional arguments", async () => {
    ...
    expect(capture.stderr).toEqual([
      ...
      "usage: rielflow hook snippet --vendor claude-code|codex|gemini",
    ]);
  });
