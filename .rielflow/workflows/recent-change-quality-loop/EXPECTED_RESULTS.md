...
```bash
nix run ./rielflow -- workflow validate recent-change-quality-loop
```
...
```bash
nix run ./rielflow -- workflow run recent-change-quality-loop \
...
```json
{
  ...
  "changedFiles": [
    "README.md",
    ".rielflow/README.md",
    ".rielflow/workflows/recent-change-quality-loop/workflow.json"
  ],
  "verification": [
